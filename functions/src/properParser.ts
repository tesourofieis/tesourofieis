...
import {
  ...
  VISIBLE_SECTIONS,
  getTranslation,
} from "./constants.ts"; // Adjust the import path accordingly
...
class ProperParser {
  ...
  private _parseProperSource(
    ...
    const parsedComment: Record<string, any> = this._parseComment(
        ...
        if (ln.match(SECTION_REGEX)) {
          ...
          if (REFERENCE_REGEX.test(ln)) {
            ...
            if (pathBit) {
              ...
              const nestedSection: Section | null =
                nestedProper.getSection(nestedSectionName);
              ...
              const nestedSectionBody: string[] =
  ...
  private _filterSections(proper: Proper) {
    ...
    const getExcludedInterReadingsSections = (
      ...
      if (
        ...
        proper.getSection(GRADUALE) !== null
      ) {
  ...
  private _addPrefaces(proper: Proper, lang: string): ParsedSource {
    const prefaceName =
      this.config.preface || proper.getRule("preface") || null;

    if (
      prefaceName === PREFATIO_OMIT ||
      (prefaceName === null && "Prefatio" in proper.keys())
    ) {
      return proper;
    }
  ...
  _get_full_path(partial_path: string, lang: string = LANGUAGE) {
    ...
    const full_path = `./functions/resources/divinum-officium-custom/web/www/missa/${DIVOFF_LANG_MAP[lang]}/${partial_path}`;
    if (!fs.existsSync(full_path)) {
      ...
      const fallback_path = `./functions/resources/divinum-officium/web/www/missa/${DIVOFF_LANG_MAP[lang]}/${partial_path}`;
