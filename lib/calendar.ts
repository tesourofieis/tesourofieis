...
class Calendar {
	...
	private fillInTemporaDays() {
		...
		this.insertBlock(this.calcSeptuagesima(this.year), FROM_PRE_LENT_TO_POST_PENTECOST);
		this.insertBlock(
		...
		this.insertBlock(this.calc24SundayAfterPentecost(this.year), WEEK_24_AFTER_PENTECOST);
		this.insertBlock(
			this.calcFirstAdventSunday(this.year),
			ADVENT,
			false,
		...
		this.insertBlock(this.calcEmberWednesdaySeptember(this.year), EMBER_DAYS_SEPTEMBER);
		...
		this.insertBlock(new Date(this.year, 11, 29), NATIVITY_OCTAVE_FERIA, false, false);
		this.insertBlock(new Date(this.year, 11, 30), NATIVITY_OCTAVE_FERIA, false, false);
		this.insertBlock(new Date(this.year, 11, 31), NATIVITY_OCTAVE_FERIA, false, false);
	}
	...
	private insertBlock(
		...
		for (const [ii, observance_ids] of block.entries()) {
			...
			if (this._container.get(dateKey)) {
				this._container.get(dateKey).tempora = [new Observance(observance_ids, dateKey)];
				this._container.get(dateKey).celebration = [...this._container.get(dateKey).tempora];
			}
		}
	}
	...
	private resolveConcurrency() {
		...
		for (const [date, day] of this._container.entries()) {
			const [celebration, commemoration, shifted] = this.applyRules(
				date,
				shiftedAll.get(date) || []
			);

			this._container.get(date).celebration = celebration;
			this._container.get(date).commemoration = commemoration;
