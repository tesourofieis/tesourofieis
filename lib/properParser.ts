...
import { ProperConfig, Proper, Section, ParsedSource } from './proper';
...
class ProperParser {
	...
	parse() {
		...
		this.prefaces[LANGUAGE] = this._parseSource('Ordo/Prefationes.txt', LANGUAGE);
		this.prefaces[LANGUAGE_LATIN] = this._parseSource('Ordo/Prefationes.txt', LANGUAGE_LATIN);
		const partial_path = this._get_partial_path();
		try {
			const proper_vernacular = this._parseProperSource(partial_path, LANGUAGE);
			const proper_latin = this._parseProperSource(partial_path, LANGUAGE_LATIN);
			return [proper_vernacular, proper_latin];
		} catch (error) {
			if (error) {
				console.error(`Proper not found. ${error} partialPath => ${partial_path}`);
			} else {
				throw error;
			}
		}
	...
	private _parseProperSource(partialPath: string, lang: string, lookupSection?: string) {
		const parsedSource = this._parseSource(partialPath, lang, lookupSection);
		...
		proper.tags = parsedComment.tags;
		proper.tags.push(this.translations[lang].PAGES.get(this.proper_id, []));
		proper.supplements = this.translations[lang].SUPPLEMENTS[this.proper_id] || [];
		this._addPrefaces(proper, lang);
		this._filterSections(proper);
		this._amendSectionsContents(proper);
		this._translateSectionTitles(proper, lang);

		return proper;
	}

	private _parseSource(partialPath: string, lang: string = LANGUAGE, lookupSection?: string) {
		...
		const fileContent: string = fs.readFileSync(fullPath || partialPath, 'utf8');
		const lines = fileContent.split('\n');

		for (let itr = 0; itr < lines.length; itr++) {
			let ln: string = lines[itr].trim();

			if (sectionName === null && ln === '') {
			...
			if (sectionName === null && REFERENCE_REGEX.test(ln)) {
				...
				const nestedPath: string = this._get_full_path(`${pathBit}.txt`, lang) || partialPath;
				if (!nestedPath) {
			...
			if (!lookupSection || lookupSection === sectionName) {
				if (ln.match(SECTION_REGEX)) {
					...
					if (REFERENCE_REGEX.test(ln)) {
						const [, pathBit, nestedSectionName] = REFERENCE_REGEX.exec(ln) || [];
						if (pathBit) {
							...
							const nestedProper = this._parseSource(nestedPath, lang, nestedSectionName);

							const nestedSection: Section | null = nestedProper.getSection(nestedSectionName);
							if (nestedSection !== null) {
								parsedSource.getSection(sectionName).extendBody(nestedSection.body);
							} else {
								console.error(
							...
							const nestedSectionBody: string[] = parsedSource.getSection(nestedSectionName).body;
							parsedSource.getSection(sectionName).extendBody(nestedSectionBody);
						}
					} else {
						// Finally, a regular line...
						// Line ending with `~` indicates that the next line should be treated as its continuation
						const appendLn: string = ln.replace(/~$/, ' ');
	...
	private _parseComment(comment: Section | null) {
		...
		for (const ln of comment.getBody()) {
			if (ln.startsWith('#')) {
				parsedComment.title = ln.trim().substring(1).replace(/[–—-]/g, '').trim();
	...
	private _normalize(ln: string, lang: string) {
		for (const { pattern, replacement } of this.translations[lang].TRANSFORMATIONS) {
	...
	private _filterSections(proper: Proper) {
		const notVisible = (sectionId: string): boolean => !(sectionId in VISIBLE_SECTIONS);
		...
		const getExcludedInterReadingsSections = (
			...
			if (config.interReadingsSection === GRADUALE && proper.getSection(GRADUALE) !== null) {
		...
		for (const exclude of getExcludedInterReadingsSections(this.config, proper)) {
			sectionsToRemove.add(exclude);
		}
	...
	private _amendSectionsContents(proper: ParsedSource) {
		...
		if (this.config.stripAlleluia && gradual !== null) {
	...
	private _translateSectionTitles(proper: Proper, lang: string) {
		proper.commemorationsNamesTranslations = this.translations[lang].COMMEMORATIONS;
		const sectionsIds = Object.keys(proper);
		let sectionLabels: Record<string, string> = {};

		sectionLabels = {
			...sectionLabels,
			...this.translations[lang].SECTION_LABELS,
		};

		if (sectionsIds.includes('GradualeL1')) {
			sectionLabels = {
				...sectionLabels,
				...this.translations[lang].SECTION_LABELS_MULTI,
			};
		}

		for (const section of Object.values(proper._container)) {
			section.setLabel(sectionLabels[section.id] || section.id);
		}
	...
	private _addPrefaces(proper: Proper, lang: string): ParsedSource {
		const prefaceName = this.config.preface || proper.getRule('preface') || null;

		if (prefaceName === PREFATIO_OMIT || (prefaceName === null && 'Prefatio' in proper.keys())) {
			return proper;
		}
	...
	private _resolveConditionals(proper: ParsedSource) {
		for (const [sectionName, section] of proper.items()) {
			...
			for (const ln of section?.body) {
				...
				if (ln.includes('(rubrica 1570 aut rubrica 1910 aut rubrica divino afflatu dicitur)')) {
	...
	_get_full_path(partial_path: string, lang: string) {
		try {
			...
			console.log('Using full path:', full_path);
			return full_path;
		} catch (error) {
	...
	_get_partial_path() {
		try {
			...
			console.error('Proper ID should follow format `<flex>:<name>`, e.g. `tempora:Adv1-0`');
		}
	}
}
