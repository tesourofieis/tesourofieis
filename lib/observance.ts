...
import { ProperParser } from './properParser';
import { Proper, ProperConfig } from './proper';
...
export class Observance {
	...
	constructor(observanceId: string, date_: string) {
		...
		if (
			...
			![COMMUNE_C_10A, COMMUNE_C_10B, COMMUNE_C_10C, COMMUNE_C_10PASC, COMMUNE_C_10T].includes(
				observanceId
			)
		) {
			this.weekday = WEEKDAY_MAPPING[parseInt(name.replace(/^.*-(\d+).*$/, '$1'), 10)];
		} else {
	...
	private adjustSundayShiftedFromPostEpiphany(propers: Proper[]): void {
		const properSunday23PostPentecost = new ProperParser(TEMPORA_PENT23_0).parse();
		for (let i = 0; i < propers.length; i++) {
			const proper = propers[i];
			for (const section of ['INTROIT', 'GRADUALE', 'OFFERTORIUM', 'COMMUNIO']) {
				proper.setSection(section, properSunday23PostPentecost[i].getSection(section));
			}
		}
	}
