...
function rule_nativity_has_multiple_masses(
	...
	if (nativityObservance) {
		const nativityMasses = observances.filter((ld) => ld.id.startsWith('sancti:12-25m'));
...
function rule_all_souls(
	...
	if (match(observances, SANCTI_11_02_1)) {
		const allSouls = observances.filter((ld) => ld.id.startsWith('sancti:11-02m'));
		if (isSunday(date_)) {
			return [
				[],
...
function rule_st_matthias(
	...
	if (match(observances, SANCTI_02_24) && isLeapYear(date_) && getDay(date_) === 24) {
		return [
...
function rule_feb27(
	...
	if (match(observances, SANCTI_02_27) && isLeapYear(date_) && getDay(date_) === 27) {
		return [
			[match(observances, PATTERN_TEMPORA)],
			[],
...
function rule_bmv_office_on_saturday(
	...
	function calcProperForGivenPeriod() {
		...
		const wednesdayInHolyWeek = calendar.findDay(TEMPORA_QUAD6_3)[0];
		if (isAfter(date_, new Date(getYear(date_), 1, 2)) && isBefore(date_, wednesdayInHolyWeek[0])) {
...
function rule_shift_conflicting_1st_class_feasts(
	...
	const _calc_target_date = (): Date => {
		...
		while (getYear(target_date) === getYear(date_)) {
			...
			const all_ranks = new Set(calendar.get(yyyyMMDD(target_date)).all.map((ld) => ld.rank));
	...
	if (first_class_feasts.length > 1) {
		const [celebration, shift_day] = first_class_feasts
			...
			.slice(0, 2);
		const to_shift: [Date, Observance[]][] = [[_calc_target_date(), [shift_day]]];
...
function rule_first_class_feast_with_sunday_commemoration(
	...
	if (match(observances, SANCTI_09_29) && match(observances, PATTERN_TEMPORA_SUNDAY_CLASS_2)) {
		return [
...
function rule_2nd_class_sunday(
	...
	const pattern_tempora_sunday_class_2 = match(observances, PATTERN_TEMPORA_SUNDAY_CLASS_2);
...
function rule_3rd_class_local_saint_celebrated_3rd_class_general_saint_commemorated(
	...
	if (thirdClassLocalSancti.length > 0) {
		const thirdClassSancti = observances.filter(
			(i) => match(i.id, PATTERN_SANCTI_CLASS_3) && !match(i.id, PATTERN_SANCTI_CLASS_3_LOCAL)
		);
...
function rule_4th_class_feria_are_removed_from_celebration(
	...
	if (fourthClassTempora) {
		...
		if (commemoration) {
			return [observances.filter((o) => o !== fourthClassTempora), [commemoration], []];
		}
...
function rule_4th_class_commemorations_are_only_commemorated(
	...
	if (fourthClassSancti) {
		const filteredObservances = observances.filter((o) => o !== fourthClassSancti);
...
function rule_general(
	...
	if (observances.length === 0) {
		...
		const [first, second] = observances
			...
			.sort(
				(a, b) =>
					a.priority - b.priority || a.rank - b.rank || a.flexibility.localeCompare(b.flexibility)
			)
			.slice(0, 2);
