import { getDate, getMonth, getYear, isSunday, subDays } from 'date-fns';
import {
...
import { ProperConfig } from './proper';
import { getCustomPreface, match, yyyyMMDD } from './utils';
...
class Day {
			...
			for (const celebrationProper of celebrationPropers) {
				for (let i = 0; i < 2; i++) {
					if (celebrationProper?.length) {
						celebrationProper[i].addCommemorations(commemorationPropers.map((j) => j[i]));
					}
				}
			}
		}
	...
	private calculateProper(observances: Observance[]) {
		if (observances.length && observances.every((i) => i.hasProper())) {
			...
			for (const observance of observances) {
				...
				const properConfig = new ProperConfig(preface, false, interReadingsSection);
			...
			const rank = observances.length && !match(observances, FERIA) ? observances[0].rank : 4;

			const preface = getCustomPreface(
				observances.length && !match(observances, FERIA) ? observances[0] : inferredObservances
			);
	...
	private inferObservance() {
		...
		while (!isSunday(date) && !(getMonth(date) === 0 && getDate(date) === 6)) {
			if (date == new Date(getYear(date), 0, 1)) {
				break;
			}
			date = subDays(date, 1); // subtract one day
			console.log(date);
		}
	...
	serialize() {
		...
		for (const container of containers) {
			serialized[container] = this[container].map((item) => item.serialize());
		}
	...
	toString(): string {
		return `${JSON.stringify(this.tempora)}${JSON.stringify(this.celebration)}${JSON.stringify(
			this.commemoration
		)}`;
	}
}
